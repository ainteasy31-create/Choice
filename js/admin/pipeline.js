(function(){
  ...
  // Mirrors the rules in scraper/enrichment.py:validate_for_publish().
  // Must pass before any pipeline record is promoted to a live listing.
  // Returns { ok: boolean, failures: string[] }.
  //
  // Image check:
  //   - If the listing already has a choice_property_id (re-publish / already
  //     published once), we query property_photos to count real ImageKit URLs.
  //   - For a first-time publish we check that original_image_urls is non-empty
  //     (source photos exist and will be transferred by import-pipeline-photos
  ...
  async function validateForPublish(listing) {
    ...
    if (listing.choice_property_id) {
      ...
      const transferred = photos?.length ?? 0;
      if (transferred === 0) {
        failures.push('No images have been transferred to ImageKit yet — use "Transfer photos" first');
      }
    } else {
      // First publish — verify source photos exist (they will be imported post-publish)
    ...
    if (freeAppRe.test(desc)) {
      failures.push('Description contains free-application language (must be "Application Fee: $50")');
    }
    ...
    feeMatches.forEach(function(m) {
      var amt = parseFloat((m.match(/(\d+(?:\.\d{2})?)$/) || [])[1] || '50');
      if (Math.abs(amt - 50) > 0.01) {
        failures.push('Description references a non-$50 application fee: found 
 ...
 + amt);
    ...
    if (tourRe.test(desc)) {
      failures.push('Description contains tour/showing/contact CTA language — remove before publishing');
    }
